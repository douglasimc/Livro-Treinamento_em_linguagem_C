/* cartao.c */
/* Imprime um cartão de Natal */
/* Estética corrigida conforme a saída prevista no livro */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main()
{
    ...
    printf("Destinatário: "); gets(Nome);
    printf("Mensagem dentro da árvore: "); gets(Mensagem);
    Tamanho=strlen(Mensagem);
    printf("Sinal interno direito:"); sDireito=getche();
    printf("\nSinal interno esquerdo:"); sEsquerdo=getche();
    Direito=38-Tamanho; Contorno[0]='/'; ns=Direito/2;
    for(i=1; i<=ns ; i++) Contorno[i]=sEsquerdo;
    strcpy(&Contorno[i], Mensagem);
    fim=strlen(Contorno);
    for(i=fim; i < fim+ns ;i++) Contorno[i]=sDireito;
    Contorno[i]='\\'; Contorno[i+1]='\0'; fim=strlen(Contorno);
